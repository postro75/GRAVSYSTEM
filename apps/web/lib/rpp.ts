...
function formatReaperMidiSource(config: MusicConfig, trackName: string, channel: number): string {
  ...
  const totalTicks = config.bars * barTicks;
  const trackEvents = eventsByTrack.get(trackName) || [];

  let source = '';
  source += `        HASDATA 1 ${ticksPerBeat} QN\n`;
  source += `        CCINTERP 32\n`;
  ...
  // Add an all-notes-off / reset at start
  source += `        E 0 b0 7b 00\n`;

  const sorted = [...trackEvents].sort((a, b) => a.time - b.time);
  for (const evt of sorted) {
    const scaledTime = Math.round((evt.time / 480) * ticksPerBeat);
    const scaledDuration = Math.max(1, Math.round((evt.duration / 480) * ticksPerBeat));
    const noteOnStatus = (0x90 | channel).toString(16).padStart(2, '0');
    const noteOffStatus = (0x80 | channel).toString(16).padStart(2, '0');
    const note = evt.note.toString(16).padStart(2, '0');
    const vel = evt.velocity.toString(16).padStart(2, '0');
    source += `        E ${scaledTime} ${noteOnStatus} ${note} ${vel}\n`;
    source += `        E ${scaledTime + scaledDuration} ${noteOffStatus} ${note} 00\n`;
  }
...
export function generateRpp(config: MusicConfig): string {
  const tracks = config.trackLayout;
  ...
  const barLength = (4 * 60) / config.bpm;
  const totalLength = config.bars * barLength;

  for (let i = 0; i < tracks.length; i++) {
    const trackName = tracks[i];
    const channel = i % 15;
    rpp += `  <TRACK ${i} ${trackName.toUpperCase().replace(/\s+/g, '_')}\n`;
    rpp += `    PEAKCOL 165176\n`;
    rpp += `    BEZ 0\n`;
    rpp += `    DEFSHAPE 0 -1 -1 -1\n`;
    rpp += `    VOLPAN 1 0 -1 -1 1\n`;
    rpp += `    MUTESOLO 0 0 0\n`;
    rpp += `    IPHASE 0\n`;
    rpp += `    PLAYOFFS 0 1\n`;
    rpp += `    ISBUS 0 0\n`;
    ...
    rpp += `    NCHAN 2\n`;
    rpp += `    FX 1\n`;
    rpp += `    TRACKID {${'01234567-89ab-cdef-0123-456789abcdef'.slice(0, 8)}${i}${'000000000000'.slice(i.toString().length)}}\n`;
    rpp += `    PERF 0\n`;
    rpp += `    MIDIOUT -1\n`;
    rpp += `    MAINSEND 1 0\n`;
    rpp += `    <FXCHAIN\n`;
    ...
    rpp += `      POSITION 0\n`;
    rpp += `      SNAPOFFS 0\n`;
    rpp += `      LENGTH ${totalLength.toFixed(6)}\n`;
    rpp += `      LOOP 1\n`;
    rpp += `      ALLTAKES 0\n`;
    ...
    rpp += `      MUTE 0 0\n`;
    rpp += `      SEL 0\n`;
    rpp += `      IGUID {${i.toString(16).padStart(8, '0')}-0000-0000-0000-000000000000}\n`;
    rpp += `      IID 1\n`;
    rpp += `      NAME ""\n`;
    rpp += `      VOLPAN 1 0 1 -1\n`;
    rpp += `      SOFFS 0\n`;
    ...
    rpp += `      CHANMODE 0\n`;
    rpp += `      GUID {${i.toString(16).padStart(8, '0')}-1111-1111-1111-111111111111}\n`;
    rpp += `      <SOURCE MIDI\n`;
    rpp += formatReaperMidiSource(config, trackName, channel);
    rpp += `      >\n`;
    rpp += `    >\n`;
    rpp += `  >\n`;
  }
